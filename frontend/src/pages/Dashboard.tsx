...
const PIE_COLORS = ['#6366f1', '#8b5cf6', '#a855f7', '#ec4899', '#f43f5e', '#f97316']
...
function CustomTooltip({ active, payload, label }: TooltipProps<number, string>) {
  ...
  return (
    <div style={{
      ...
      {label && <div style={{ color: '#888', marginBottom: 6 }}>{label}</div>}
      {payload.map((p, i) => (
        <div key={i} style={{ color: p.color, marginBottom: 2 }}>
          {p.name}: <strong>{EUR(Math.abs(Number(p.value)))}</strong>
        </div>
      ))}
    </div>
  )
...
export default function DashboardPage() {
  ...
  const analyzeMutation = useMutation({
    mutationFn: () => api.analyze(selectedPeriod!),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['report', selectedPeriod] }),
  })

  const status404 = (reportError as { response?: { status?: number } })?.response?.status === 404
  ...
  return (
    <Layout style={{ minHeight: '100vh', background: bg }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {periodsLoading ? <Spin size="small" /> : (
            <Select
              ...
              options={[...periods].sort().reverse().map(p => ({
                value: p,
                label: fmtPeriod(p),
              }))}
          ...
          <Tooltip title="Refresh report">
            <Button
              icon={<ReloadOutlined />}
              onClick={() => queryClient.invalidateQueries({ queryKey: ['report', selectedPeriod] })}
      ...
      <Content style={{ padding: '24px 32px', maxWidth: 1400, margin: '0 auto', width: '100%' }}>
        ...
        {/* No period selected */}
        {!selectedPeriod && !periodsLoading && periods.length === 0 && (
        ...
        {/* 404 — no report for this period yet */}
        {!reportLoading && status404 && selectedPeriod && (
          <div style={{ textAlign: 'center', marginTop: 80 }}>
            <Title level={3} type="secondary">No report for {fmtPeriod(selectedPeriod)}</Title>
        ...
        {report && (
          <Space direction="vertical" size={24} style={{ width: '100%' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
              <Title level={3} style={{ margin: 0 }}>{fmtPeriod(report.period)}</Title>
              {report.generatedAt && (
                ...
                <Col key={m.label} xs={12} sm={8} md={4}>
                  ...
                  >
                    <Statistic
                      ...
                      value={m.isCurrency === false ? m.value : m.suffix === '%' ? m.value : m.value}
            ...
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={14}>
                <Card title="P&L Overview" style={{ height: '100%' }}>
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart data={pnlChartData} margin={{ top: 4, right: 16, left: 8, bottom: 4 }}
                      barCategoryGap="30%">
                ...
                <Card title="Expense Breakdown" style={{ height: '100%' }}>
                  <ResponsiveContainer width="100%" height={260}>
                    <PieChart>
                      ...
                      <ReTooltip formatter={(v: number) => EUR(v)} contentStyle={{
                        background: '#1a1a2e', border: '1px solid #333', borderRadius: 8,
                      }} />
