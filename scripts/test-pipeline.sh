...
if [[ "${CI_SKIP_EXTRACTION:-0}" == "1" ]]; then
  echo "[2/5] Upload — SKIPPED (CI_SKIP_EXTRACTION=1, pre-seeded demo docs used)"
  echo "[3/5] Extraction job — SKIPPED (CI_SKIP_EXTRACTION=1)"
  echo "[4/5] Job poll — SKIPPED (CI_SKIP_EXTRACTION=1)"
else
