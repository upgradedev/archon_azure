  ...
  properties: {
    ...
    configuration: {
      ...
      secrets: [
        ...
        // keyVaultUrl uses secretUri so ARM implicitly depends on the KV secret
        // resources being created before the Container App revision is provisioned.
        { name: 'storage-conn', keyVaultUrl: kvSecretStorageConn.properties.secretUri, identity: 'system' }
        { name: 'openai-key',   keyVaultUrl: kvSecretOpenaiKey.properties.secretUri,   identity: 'system' }
        { name: 'search-key',   keyVaultUrl: kvSecretSearchKey.properties.secretUri,   identity: 'system' }
        { name: 'pg-url',       keyVaultUrl: kvSecretPgUrl.properties.secretUri,       identity: 'system' }
        ...
        { name: 'foundry-conn', keyVaultUrl: kvSecretFoundryConn.properties.secretUri, identity: 'system' }
      ]
    }
    template: {
      containers: [{
